...
def log_memory_usage():
    ...
    print(f'Memory usage: RSS={mem_info.rss / (1024 * 1024)} MB, VMS={mem_info.vms / (1024 * 1024)} MB')
...
class MyDataFrame:
    ...
    def slice(self, start=None, end=None, keep=True, view=False):
        ...
        if view:
            return MyDataFrameView(new_column_names, sliced_data)
        else:
            return MyDataFrame({col: sliced_data[:, i].tolist() for i, col in enumerate(new_column_names)})
    ...
    def rename_column(self, old_name, new_name, contains=None):
        if contains:
            ...
            if old_name not in self.column_names:
                raise ValueError(f"Column '{old_name}' does not exists in the DataFrame.")
    ...
    def merge(self, other, on, how='inner', suffix=('_df1', '_df2'), indicator=False):
        ...
        if missing_columns_self or missing_columns_other:
            raise ValueError(f"Some columns do not exist in the DataFrame: {', '.join(missing_columns_self + missing_columns_other)}")
...
class MyDataFrameView(MyDataFrame):
...
data = {
    'id': ['1', '1', '2', '3', '4'], #[random.randint(0, 999999) for _ in range(random_range)],
    'name': ['Alice', 'Alice', 'Bob', 'Charlie', 'David'],#[''.join(random.choices(string.ascii_letters, k=random.randint(1, 24))) for _ in range(random_range)],
    'amount': ['100.5', '101.4', '200.75', '300.25', '400.0'],#[round(random.uniform(0, 99.9), 2) for _ in range(random_range)],
    'sv_op_timestamp': [(datetime(2020, 1, 1) + timedelta(days=random.randint(0, (datetime.now() - datetime(2020, 1, 1)).days), seconds=random.randint(0, 24 * 60 * 60 - 1))).strftime('%Y-%m-%d %H:%M:%S') 
        for _ in range(random_range)]
}

df = MyDataFrame(data, chunk_size=1000)
if not df.empty():
    print(len(df))
log_memory_usage()
print("Original Data:\n", df.get_data())
...
for _, (pk_c, pk_n) in enumerate(zip(pk_columns + sort_columns, new_pk)):
    df.add_column(pk_n, values=[str(pk).lower() for pk in df.get_column(pk_c)])
log_memory_usage()
print("After Adding Column:\n", df.get_data())
print()

# Sort by a list of columns
df.sort(new_pk, ascending=[True, True, False])
log_memory_usage()
print("After Sorting:\n", df.get_data())
print()

new_ascending = new_pk[:-len(sort_columns)]

# Group by a list of columns
df.groupby(new_ascending, first=True)
log_memory_usage()
...
# Modify a column
df.modify_column('name', lambda x: x.lower())
log_memory_usage()
print("After Modifying Column 'name':\n", df.get_data())
print()

#Slice in Batches
#batch_size = 3
#for batch in df.slice_batch(batch_size):
#    print(batch.get_data())
#log_memory_usage()
...
df_csv = MyDataFrame.read_csv(file_like, delimiter=chr(1), na_values=['NULL'], dtype=str, fillna='ACTUAL_NULL', chunk_size=1000)
log_memory_usage()
print(f"CSV DataFrame: {df_csv.get_data()}")

df_merge = df.merge(df_csv, on=pk_columns, how='inner', suffix=('_etl_hive', '_etl_sql'), indicator=True)

log_memory_usage()
print("After Merge:")
print(df_merge.get_data())

df_merge.rename_column(old_name='_etl_hive', new_name='', contains='_etl_hive')
